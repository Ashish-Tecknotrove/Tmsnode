...
class Middleware {
    ...
    generateAuth(payload, userType) {
        return __awaiter(this, void 0, void 0, function* () {
            if (userType == 1) {
                ...
                return (0, jsonwebtoken_1.sign)(payload, process.env.jwt_branch_secreate, { expiresIn: 43200 });
            }
            else if (userType == 6) {
                return (0, jsonwebtoken_1.sign)(payload, process.env.jwt_department_secreate, { expiresIn: 43200 });
            }
        });
    }
    ...
    verify_branchAdmin_AuthenticateToken(req, res, next) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const authheader = req.header('authorization');
                const token = authheader && authheader.split(" ")[1];
                if (token == null)
                    return res.status(response_codes_1.default.UNAUTHORIZED).json({ response_code: 0, message: "Oops! we cannot process the request without authentication token" });
                yield (0, jsonwebtoken_1.verify)(token, process.env.jwt_branch_secreate, (err, user) => {
                    if (err) {
                        return res.status(response_codes_1.default.UNAUTHORIZED).json({ response_code: 0, message: err.message });
                    }
                    else {
    ...
    verify_departmentAdmin_AuthenticateToken(req, res, next) {
        return __awaiter(this, void 0, void 0, function* () {
            try {
                const authheader = req.header('authorization');
                const token = authheader && authheader.split(" ")[1];
                if (token == null)
                    return res.status(response_codes_1.default.UNAUTHORIZED).json({ response_code: 0, message: "Oops! we cannot process the request without authentication token" });
                yield (0, jsonwebtoken_1.verify)(token, process.env.jwt_department_secreate, (err, user) => {
                    if (err) {
                        return res.status(response_codes_1.default.UNAUTHORIZED).json({ response_code: 0, message: err.message });
                    }
                    else {
