...
import ElearningTrainingSession from "../../../../core/model/elearning/elearning_training_session.model";
import responseStrings from "../../../../core/strings/response-strings";
import Trainee from "../../../../core/model/root/trainee.model";
import ElearningTraineeScromData from "../../../../core/model/elearning/elearning_trainee_scrom_data.model";
...
class ElearningTraineeTest {
    ...
    async start_training_session(req: Request, res: Response) {
        ...
        try {
            ...
            await
                ElearningTrainingSession.findAll({
                    where: {
                    ...
                    if (sessionData.length == 0) {
                        ...
                        await ElearningTrainingSession.create({...TraineeData}).then(async (newSession) => {
                            ...
                            await ElearningTrainingSession.update({...update}, {where: {id: newSession['id']}}).then(completed => {
                                const filePath =
                                    new URL(urlHeader + '://' + req.get('host') + "/resources/course/");
